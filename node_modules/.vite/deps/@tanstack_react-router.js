...
function getElementScrollRestorationEntry(router, options) {
  ...
  if (options.id)
    return (_a = scrollRestorationCache == null ? void 0 : scrollRestorationCache.state[restoreKey]) == null ? void 0 : _a[`[${scrollRestorationIdAttribute}="${options.id}"]`];
...
function setupScrollRestoration(router, force) {
  ...
  if ((isServer ?? router.isServer) || router.isScrollRestorationSetup || !cache)
    return;
    ...
    this.buildLocation = (opts) => {
      const build = (dest = {}) => {
        ...
        if (destRoute && !opts.leaveParams)
          try {
            if (((_b = this.getMatchedRoutes(nextPathname).foundRoute) == null ? void 0 : _b.id) !== destRoute.id)
              console.warn(`Generated path "${nextPathname}" for route "${destRoute.id}" did not match the same route after params.stringify.`);
          } catch {
          }
  ...
  for (const route of destRoutes) {
    ...
    if (route.options.validateSearch) {
      const validate = ({ search, next }) => {
        ...
        if (!context._includeValidateSearch)
          return result;
        try {
          return {
            ...result,
